...
### 🔍 Kesimpulan Utama
* **Kompensasi Bias & Superioritas Konteks Lokal:** Integrasi *Zero-Shot LLM* sukses mengekstrak sentimen ulasan objektif guna memitigasi *rating bias* di platform Google Maps. IndoBERT memenangi metrik utama **Macro F1-Score (0.7288)**. Model monolingual terbukti jauh lebih sensitif (*Macro Recall* = **0.6970**) dalam mendeteksi ekspresi kekecewaan atau keluhan lokal karena memiliki basis korpus kosa kata bahasa Indonesia yang kaya. Sebaliknya, **XLM-RoBERTa bersikap lebih konservatif**, namun memiliki tingkat kepastian tebakan paling akurat dengan capaian *Macro Precision* tertinggi (**0.8957**).
* **Validitas Transferabilitas Lintas Domain:** Penelitian ini membuktikan bahwa model NLP, khususnya IndoBERT, tetap bagus saat ditransfer langsung ke domain baru (Wisata Budaya & Sejarah) dengan capaian Macro F1-Score sebesar 0.7288. Keberhasilan transfer pengetahuan lintas domain ini didasari oleh hubungan sebab-akibat yang kuat, di mana kualitas pembersihan data oleh LLM di fase awal memberikan pondasi semantik yang bersih, sehingga model akhir mampu menjembatani perbedaan kosakata antar-kluster secara optimal.
* **Efisiensi Implementasi Sektor Publik:** Penerapan metode adaptasi parameter (LoRA) membuktikan bahwa sistem pemantauan ulasan otomatis lintas kluster wisata dapat dibangun secara optimal tanpa infrastruktur komputer yang mahal. Kombinasi ini memberikan solusi teknologi efisien bagi instansi pariwisata daerah.
---
